...
type: execute
depends_on: []
files_modified: [src/bsa/envs/virtualhome/env.py, src/bsa/envs/virtualhome/__init__.py, scripts/install_virtualhome.py, scripts/setup_venv.py, pyproject.toml, requirements.txt, requirements-virtualhome.txt]
domain: python
---
...
<task type="auto">
  <name>Task 1: Create virtual environment setup script</name>
  <files>scripts/setup_venv.py, requirements.txt, requirements-virtualhome.txt, pyproject.toml</files>
  <action>
    Create `scripts/setup_venv.py` that:
    1. Checks for Python 3.9 or 3.10 (preferred for VirtualHome compatibility)
    2. Creates a virtual environment in `venv/` directory
    3. Installs base project dependencies with NumPy <2.0 constraint (for VirtualHome compatibility)
    4. Optionally installs VirtualHome with compatible dependencies
    5. Provides clear activation instructions
    
    Update `pyproject.toml` to pin NumPy to <2.0.0 for VirtualHome compatibility.
    Update `requires-python` to ">=3.9,<3.12" to ensure compatibility.
    
    Create `requirements.txt` with base dependencies (NumPy <2.0).
    Create `requirements-virtualhome.txt` with VirtualHome-specific dependencies.
    
    The script should handle:
    - Finding appropriate Python version (3.9 or 3.10 preferred)
    - Creating venv if it doesn't exist
    - Installing dependencies in correct order
    - Verifying VirtualHome installation if requested
  </action>
  <verify>
    Run `python scripts/setup_venv.py` - should create venv and install dependencies. Check that NumPy version is <2.0 in venv.
  </verify>
  <done>Virtual environment setup script exists, requirements files created, pyproject.toml updated with compatibility constraints</done>
</task>

<task type="auto">
  <name>Task 2: Research VirtualHome Python package and create installation script</name>
...
<task type="auto">
  <name>Task 3: Create VirtualHomeEnvironment class implementing Environment interface</name>
