#!/usr/bin/env python3
"""Run Phase 11 experiments with Realism & Anti-Cheating fixes.

# Phase 11 Changes:
# 1. Helper uses HelperWorldModel (no ground truth access)
# 2. Scaled up experiments (10 runs x 100 episodes = 1000 evals per condition)
# 3. Maintains all statistical rigor from Phase 10
...
def main():
    parser = argparse.ArgumentParser(description="Run Phase 11 experiments")
    ...
    print("=" * 70)
    print("Phase 11: Realism & Anti-Cheating Experiments")
    print("=" * 70)
    print(f"Episodes per condition: {args.episodes}")
    print(f"Runs per config: {args.runs}")
    print(f"Total Evaluations: {args.episodes * args.runs * 3 * 3}")
    print()
    ...
    for condition, drift_prob in conditions.items():
        ...
        for run_idx in range(args.runs):
            base_seed = 20000 + run_idx * 1000  # New seed range for Phase 11
            ...
            for ep_idx in range(args.episodes):
                ...
                for model_name in models:
                    ...
                    result = {
                        ...
                        # Detailed wasted action breakdown
    ...
    print("\n" + "=" * 70)
    print("Results Summary (Phase 11: Realism & Anti-Cheating)")
    print("=" * 70)
    ...
    if len(belief_auroc) > 0:
        if belief_auroc.mean() >= 0.999:
            ...
            print(f"  OK: belief_pf AUROC = {belief_auroc.mean():.3f} ± {belief_auroc.std():.3f} (Realistic)")
    ...
    # Save results
    output_dir = Path("results/metrics/phase11_realism")
